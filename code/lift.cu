...
//Performs dot_prod = vij * Q_T
__global__ void MatMul(float vij[], float Q[], float dot_prod[]) {
    int vij_ind = threadIdx.x*R;
    int Q_ind = threadIdx.y*M;
    int id = threadIdx.x*M + threadIdx.y; 
    for (int i=0; i<R; i++) {
        dot_prod[id] += vij[vij_ind+i]*Q[Q_ind+i];
    }
}
        ...
        for (int m=0; m<MAX_ITER; m++) 
        {
...

            {
                ...
                } else 
                {
                    ...
                    if(cudaMalloc(&d_vij,(sizeof(float))*J*R) != cudaSuccess){
                        cout<<"Could not allocate d_vij"<<endl;
                    }
                    
                    if(cudaMalloc(&d_Q,(sizeof(float))*M*R) != cudaSuccess){
                        cout<<"Could not allocate d_Q"<<endl;
                    }

                    if(cudaMalloc(&d_dot_prod,(sizeof(float))*J*M) != cudaSuccess){
                        cout<<"Could not allocate d_dot_prod"<<endl;
                    }
                    
                    if(cudaMemcpy(d_vij, &vij, sizeof(float)*J*R,cudaMemcpyHostToDevice) != cudaSuccess){
                    cout<<"Could not copy vij into d_vij"<<endl;
                    }
                    
                    if(cudaMemcpy(d_Q, &Q, sizeof(float)*M*R,cudaMemcpyHostToDevice) != cudaSuccess){
                        cout<<"Could not copy Q into d_Q"<<endl;
                    }

                    dim3 blockDim(J,M,1);
                    MatMul<<<1, blockDim>>>(d_vij, d_Q, d_dot_prod);
                    if(cudaMemcpy(&dot_prod, d_dot_prod, sizeof(float)*M*J,cudaMemcpyDeviceToHost) != cudaSuccess){
                        cout<<"Could not copy d_dot_prod into dot_prod"<<endl;
                    }
